...
func _draw():
	...
	for y in range(get_viewport().size.y / grid_steps[curr_grid_size][0]):
		var grid_step = grid_steps[curr_grid_size][0]
		for yy in range(1, 5):
			var small_grid_step = grid_steps[curr_grid_size][1]
	...
	for x in range(1, get_viewport().size.x / grid_steps[curr_grid_size][0]+1):
		var grid_step = grid_steps[curr_grid_size][0]
		draw_line(Vector2(x*grid_step, 0), Vector2(x*grid_step, get_viewport().size.y), Color.PURPLE, -2)
	for y in range(1, get_viewport().size.y / grid_steps[curr_grid_size][0]+1):
		var grid_step = grid_steps[curr_grid_size][0]
		draw_line(Vector2(0, y*grid_step), Vector2(get_viewport().size.x, y*grid_step), Color.PURPLE, -2)


func _on_camera_control_zoom_changed(zoom: float, min_zoom:float, max_zoom:float)->void:
